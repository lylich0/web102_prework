...
function addGamesToPage(games) {
    ...
    // loop over each item in the data
        for (let game of games) {
            ...
            elem.innerHTML = `<img src=${game.img} class="game-img">
            <h3>${game.name}</h3>
            <div>${game.description}</div>
            `;

            // append the game to the games-container
            gamesContainer.append(elem);
        }
}
...
let totalAmount = GAMES_JSON.reduce((acc, game) => {
...
let displayStr = `
    A total of $${totalAmount.toLocaleString("en-US")} has been raised for ${totalGames} games. ${unfundedGames.length === 0 ? 
    'All games are funded!': `Currently, ${unfundedGames.length} ${unfundedGames.length > 1 ? 'games' : 'game'} remains unfunded.`}
`
...
let firstGameElem = document.createElement("div");
firstGameElem.innerHTML = firstGame.name;

firstGameContainer.append(firstGameElem);
// do the same for the runner up item
let secondGameElem = document.createElement("div");
